...
const APP_NAME = import.meta.env.VITE_APP_NAME || 'AICOUNTLY QA Portal'
      ...
      <div className="w-full max-w-md">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-neutral-500">qa.aicountly.org · Console identity only</div>
          </div>
        </div>
        ...
        <div className="qa-card">
          {isPending ? (
            ...
            <>
              ...
              <p className="mt-2 text-sm text-neutral-600">
                You are signed in to Console, but this account does not have access to the QA controller app.
              </p>
              {gateMessage ? (
                <div className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-900">{gateMessage}</div>
              ) : null}
              <p className="mt-3 text-xs text-neutral-500">
                Ask a Console administrator to grant QA access under Controller App Access, then click Retry.
              </p>
            </>
          ) : reason === GATE_ERROR ? (
            <>
              ...
              <p className="mt-2 text-sm text-neutral-600">
                {gateMessage || 'Could not complete Console sign-in for QA Portal.'}
              </p>
            </>
          ) : (
            <>
              ...
              <p className="mt-2 text-sm text-neutral-600">
                ...
                <strong>console.aicountly.org</strong>, then open QA from Top Controller Apps or return here.
              </p>
          ...
          <div className="mt-5 flex flex-col gap-2 sm:flex-row">
            {reason === GATE_CONSOLE_REQUIRED ? (
              <a href={consoleLoginUrl()} className="qa-btn-primary justify-center text-center">
                Open Console sign-in
              </a>
            ) : null}
            <button
              type="button"
              className="qa-btn-secondary justify-center"
              onClick={() => retryAuth()}
              disabled={isPending}
            >
