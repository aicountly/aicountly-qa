...
/**
 * Thin HS256 JWT wrapper for the QA portal's independent auth.
 * Token TTL is configured via QA_JWT_TTL_MINUTES (default 720 = 12h).
 */
class Jwt
{
    ...
    {
        $secret  ??= (string) env('QA_JWT_SECRET', '');
        $minutes = $ttlMinutes ?? (int) env('QA_JWT_TTL_MINUTES', 720);

        if ($secret === '' || strlen($secret) < 32) {
            throw new RuntimeException('QA_JWT_SECRET must be at least 32 characters. Set it in .env.');
        }
